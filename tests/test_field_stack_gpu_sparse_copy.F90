...
PROGRAM TEST_FIELD_STACK_GPU_SPARSE_COPY
    ...
    CLASS(FIELD_2RB), POINTER :: F_2DB => NULL()
    ...
    !...The status of the remaining fields must be unchanged
    IF (IAND (F_4DB%GET_STATUS (), NHSTFRESH) /= NHSTFRESH) THEN
      CALL FIELD_ABORT('F_4DB SHOULD STILL BE FRESH ON HOST')
    ENDIF
    IF (IAND (F_3DB%GET_STATUS (), NHSTFRESH) /= NHSTFRESH) THEN
      CALL FIELD_ABORT('F_3DB SHOULD STILL BE FRESH ON HOST')
    ENDIF
    ...
    IF (IAND (F_BLOCKED_BUFFER%GET_STATUS (), NHSTFRESH) /= NHSTFRESH) THEN
      CALL FIELD_ABORT('F_BLOCKED_BUFFER SHOULD STILL BE FRESH ON HOST')
    ENDIF
...
END PROGRAM TEST_FIELD_STACK_GPU_SPARSE_COPY
