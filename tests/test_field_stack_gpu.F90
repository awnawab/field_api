...
PROGRAM TEST_FIELD_STACK_GPU
    ...
    IF (IAND(F_BUFFER%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_BUFFER STATUS SHOULD BE NHSTFRESH")
    IF (IAND(F_1DF%ISTATUS, NDEVFRESH) /= NDEVFRESH) CALL FIELD_ABORT("F_1DF STATUS SHOULD BE NDEVFRESH")
    IF (IAND(F_3DF%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_3DF STATUS SHOULD BE NHSTFRESH")
    IF (IAND(F_2DF%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_2DF STATUS SHOULD BE NHSTFRESH")
    ...
    IF (IAND(F_BUFFER%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_BUFFER STATUS SHOULD BE NHSTFRESH")
    IF (IAND(F_1DF%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_1DF STATUS SHOULD BE NHSTFRESH")
    IF (IAND(F_3DF%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_3DF STATUS SHOULD BE NHSTFRESH")
    IF (IAND(F_2DF%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_2DF STATUS SHOULD BE NHSTFRESH")
    ...
    !...This should have triggered data movement on the full field stack
    !...thus every field should have status NDEVFRESH = 1
    IF (IAND(F_BLOCKED_BUFFER%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_BLOCKED_BUFFER STATUS SHOULD BE NHSTFRESH")
    IF (IAND(F_2DB%ISTATUS, NDEVFRESH) /= NDEVFRESH) CALL FIELD_ABORT("F_2DB STATUS SHOULD BE NDEVFRESH")
    IF (IAND(F_4DB%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_4DB STATUS SHOULD BE NHSTFRESH")
    IF (IAND(F_3DB%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_3DB STATUS SHOULD BE NHSTFRESH")
    ...
    !...thus every field should have status NHSTFRESH = 2
    IF (IAND(F_BLOCKED_BUFFER%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_BLOCKED_BUFFER STATUS SHOULD BE NHSTFRESH")
    IF (IAND(F_2DB%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_2DB STATUS SHOULD BE NHSTFRESH")
    IF (IAND(F_4DB%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_4DB STATUS SHOULD BE NHSTFRESH")
    IF (IAND(F_3DB%ISTATUS, NHSTFRESH) /= NHSTFRESH) CALL FIELD_ABORT("F_3DB STATUS SHOULD BE NHSTFRESH")
